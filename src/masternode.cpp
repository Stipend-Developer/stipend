...
CMasternodeBroadcast::CMasternodeBroadcast(CService newAddr, CTxIn newVin, CPubKey pubKeyCollateralAddressNew, CPubKey pubKeyMasternodeNew, int protocolVersionIn)
{
    vin = newVin;
    addr = newAddr;
    pubkey = pubKeyCollateralAddressNew;
    pubkey2 = pubKeyMasternodeNew;
    sig = std::vector<unsigned char>();
    activeState = MASTERNODE_ENABLED;
    sigTime = GetAdjustedTime();
    lastPing = CMasternodePing();
...
CMasternodeBroadcast::CMasternodeBroadcast(const CMasternode& mn)
{
    ...
    donationAddress = mn.donationAddress;
    donationPercentage = mn.donationPercentage;
...
bool CMasternodeBroadcast::Create(std::string strService, std::string strKeyMasternode, std::string strTxHash, std::string strOutputIndex, std::string& strErrorRet, CMasternodeBroadcast& mnbRet, bool fOffline)
{
    ...
    if (!fOffline && !darkSendPool.IsBlockchainSynced()) {
...
bool CMasternodeBroadcast::CheckDefaultPort(std::string strService, std::string& strErrorRet, std::string strContext)
{
    ...
    if (service.GetPort() != nDefaultPort) {
        strErrorRet = strprintf("Invalid port %u for masternode %s, only %d is supported",
...
bool CMasternodeBroadcast::CheckAndUpdate(int& nDos)
{
    ...
    if (pmn->pubkey == pubkey && !pmn->IsBroadcastedWithin(MASTERNODE_MIN_DSEE_SECONDS)) {
...
bool CMasternodePing::CheckAndUpdate(int& nDos, bool fRequireEnabled)
{
    ...
    if (pmn != NULL && pmn->protocolVersion >= masternodePayments.GetMinMasternodePaymentsProto()) {
        ...
        if (!pmn->IsPingedWithin(MASTERNODE_MIN_DSEE_SECONDS - 60, sigTime)) {
            std::string strMessage = vin.ToString() + blockHash.ToString() + boost::lexical_cast<std::string>(sigTime);
